...
def pack_frames(framestrips, nt):
    ...
    for strips in framestrips:
        ...
        for palette, tiles, x, y in strips:
            ...
            b = bytearray([])
            for q in nt[ntoffset:ntend]:
                if (((q & 0x6000) >> 7) | (q & 0x3F)) != 0:
                    b.extend([y+128])
                    b.extend([x+128])
                    b.extend([(((q & 0x6000) >> 7) | (q & 0x3F)) + 0x20])
                    b.extend([(palette | ((ntiles - 1) << 5) | 0b1000) & 0x7f])
                x = x + 8
            ntoffset = ntend
            if bytes(b) != b'':
...
def emit_frames(framestrips, nt, framenames):
    out = ["; Sprite definition file: " + n for n in framenames]
    ntoffset = 0
    ...
    for thisframenames, framedef in allframedefs.values():
        #out.extend("Sprite_%s:" % framename for framename in thisframenames)
        #print(framedef)
        out.extend(
    ...
    #out.extend("    def    F_%s = %d" % (n, i) for i, n in enumerate(framenames))
    return "\n".join(out)
