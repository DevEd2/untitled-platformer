...
MONSTER_HITBOXWIDTH         equ 6
MONSTER_HITBOXHEIGHT        equ 6
...
.skiphoriz
    ...
    add     hl,bc
    ld      [hl],1
:   ; make player bounce if airborne
    ; TODO: skip if we're doing a ground pound
    ld      a,[Player_MovementFlags2]
    bit     bPlayerIsAirborne,a
    jr      z,:+
    ld      a,[sys_btnHold]
...
:   jp      KillMonster
.highjump
    ld      a,high(Player_HighJumpHeight)
    ld      [Player_YVelocity],a
    ld      a,low(Player_HighJumpHeight)
    ld      [Player_YVelocityS],a
    jp      KillMonster
...
.xMoveDone:
  ...
  add hl,bc
  ld  a,[hl]
  sub MONSTER_HITBOXWIDTH
  ld  e,a
  push  af
  ld  hl,Monster_YPosition
  add hl,bc
  ld  a,[hl]
  sub MONSTER_HITBOXHEIGHT
  ld  h,e
  ld  l,a
  ld  a,c
  ldh [Temp1],a
...
:
  ...
  jr  nz,.nextMonster
  ld  a,e
  cp  MONSTER_HITBOXWIDTH+Player_HitboxWidth
  jr  nc,.nextMonster
  ld  hl,Monster_YPosition
  add hl,bc
  ld  a,[Player_YPos]
  ...
  add 1
:
  cp  MONSTER_HITBOXHEIGHT+Player_HitboxHeight
  jr  nc,.nextMonster
  ld  hl,Monster_Collision
  add hl,bc
